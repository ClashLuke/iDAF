...
class generator():
    def __init__(self, batchsize, txt, outputs, indexIn, inputs, steps, charDictList,
                 charDict, classes, changePerKerasEpoch, embedding):
    ...
    def inpGenerator(self):
        ...
        if self.outputs == 1:
            ...
            if self.indexIn:
                ...
                tmpIn[-1][:] = [self.charDictList[self.txt[j]] for j in
                                range(self.inputs)]
                tmpOut[0][0] = self.charDict[self.txt[self.inputs]]
                while True:
                    for b in range(self.batchsize):
                        tmpIn[b][:] = np.append(tmpIn[b - 1][1:], self.charDictList[
                ...
                tmpIn = np.zeros((self.batchsize, self.inputs * self.classes),
                                 dtype=np.float32)
                ...
                while True:
                    for b in range(self.batchsize):
                        tmpIn[b][:] = np.append(tmpIn[b - 1][self.classes:],
                                                self.charDictList[
                                                    self.txt[self.inputs + b + n]])
                        tmpOut[b][0] = self.charDict[self.txt[self.inputs + 1 + b + n]]
                    for b in range(self.batchsize):
            ...
            if self.indexIn:
                ...
                tmpIn[-1][:] = [self.charDictList[self.txt[j]] for j in
                                range(self.inputs)]
                tmpOut[0][:] = [[self.charDict[self.txt[self.inputs + j]]] for j in
                                range(self.outputs)]
                while True:
                    for b in range(self.batchsize):
                        tmpIn[b][:] = np.append(tmpIn[b - 1][1:], self.charDictList[
                            self.txt[self.inputs + b + n]])
                        tmpOut[b][:] = np.append(tmpOut[b - 1][1:], self.charDict[

...
def getHiddenLayers(layer, layerCount, neuronList, leakyRelu, batchNorm, concatDense,
...
def getOutput(layer, concatBeforeOutput, outputs, classes, outputActivation, loss,
              twoDimensional):
    if twoDimensional:
        layer = tf.keras.layers.Flatten()(layer)
...
def compileModel(inp, layer, learningRate, drawModel, loss, metric, modelCompile):
    ...
    if modelCompile:
        model.compile(loss=loss, optimizer=tfa.optimizers.LAMB(lr=learningRate,
                                                               weight_decay_rate=learningRate / 100),
                      metrics=[metric])
        model.summary()
        if drawModel:
...
def getModel(leakyRelu=True, batchNorm=True, trainNewModel=True,
    ...
    if neuronList is None:
        neuronList = utils.getNeuronList(neuronsPerLayer, layerCount, classNeurons,
                                         classes)
    else:
    ...
    if trainNewModel:
        ...
        if indexIn:
            ...
            layer, inp = getInitialBinaryLayer(initialLSTM, gpu, bidirectional, inputs,
        ...
        layer = getOutput(layer, concatBeforeOutput, outputs, classes, outputActivation,
                          loss, twoDimensional)
